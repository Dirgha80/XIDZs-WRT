   ...
   for pattern in "${search_replace_patterns[@]}"; do
        ...
        for file in *"${search}"*.img.gz; do
            if [[ -f "$file" ]]; then
                ...
                local new_name
                if [[ -n "$kernel" ]]; then
                    new_name="HJ-WRT-${BRANCH}-${replace}-${kernel}-${TUNNEL}.img.gz"
                else
                    new_name="HJ-WRT-${BRANCH}-${replace}-${TUNNEL}.img.gz"
                fi
        ...
        for file in *"${search}"*.tar.gz; do
            if [[ -f "$file" ]]; then
                local new_name
                new_name="$HJ-WRT-${BRANCH}-${replace}-${TUNNEL}_Build_By_Fidz.tar.gz"
                echo -e "${INFO} Renaming: $file → $new_name"
                mv "$file" "$new_name" || {
                    echo -e "${WARN} Failed to rename $file"
                    continue
